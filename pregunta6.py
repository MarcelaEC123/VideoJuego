...
class Acertijo(pygame.sprite.Sprite):
    ...
    acertijos = []
    preguntas = ["¿Cuál es el símbolo químico del agua?"]
    ...
    def generar_acertijos(cls, cantidad, size=(150, 80), color=WHITE):
        ...
        for i in range(cantidad):
            ...
            if i < len(cls.preguntas):
                acertijo = cls(cls.preguntas[i], ["CO2", "H2O", "O2"], cls.respuesta_correcta, pos, size, color)
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            if self.rect.collidepoint(mouse_pos):
                if self.is_correct:
                    ...
                    subprocess.Popen(["python", "final.py"])  # Ejecutar final.py
                else:
